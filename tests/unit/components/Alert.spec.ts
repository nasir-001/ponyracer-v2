...
const Test = defineComponent({
  ...
  template: '<div><Alert :variant="variant" :dismissible="isDismissible" @dismissed="isDismissed = false">Hello</Alert></div>'
});
...
describe('Alert.vue', () => {
  ...
  test('should have the correct CSS class', async () => {
    const wrapper = testWrapper();

    // the alert should have the CSS classes `alert alert-danger` for the `danger` variant
    expect(wrapper.find('div.alert.alert-danger').exists()).toBe(true);

    await wrapper.setProps({ variant: 'success' });
    // the alert should have the CSS classes `alert alert-danger` for the `success` variant
